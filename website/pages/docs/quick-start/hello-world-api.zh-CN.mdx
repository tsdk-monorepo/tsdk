...
```ts filename="src/gen-route.ts" copy
...
```ts filename="src/Hello.apiconf.ts" copy
import { z } from 'zod';
...
```ts copy filename="src/Hello.api.ts"
...
<Callout type="info">`src/main.ts` 上面已创建好，我们只需要更新入口文件即可。</Callout>

```ts filename="src/main.ts" {7, 67}
import http from 'http';
import { socketIOAdapterFactory } from 'tsdk-server-adapters/lib/socket.io-adapter';
...
### 导出模块 packages/fe-sdk

执行以下命令导出模块到 **packages/fe-sdk** 文件夹
