...
In [previous step](./setup), we prepared the TypeScript project environment.
...
<Callout>
  Don't panic! You can use built-in snippets to generate code quickly [Check
  here](../guide/tsdk-snippets)
</Callout>
...
<Callout type="info">`server/src/main.ts` already create, then just update entry inside</Callout>
