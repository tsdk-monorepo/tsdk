...
tsdk 是一款 TypeScript 项目开发工具。通过 tsdk，开发人员可以更轻松地在项目间共享代码，并确保类型的一致性，从而减少潜在的错误，提高开发效率。

## 主要特性

- 类型安全：确保前后端通信以及不同模块之间的类型一致性
- [代码共享](/docs/guide/share-code)：简化在项目不同部分之间共享代码的流程
- 代码生成：生成直接接口调用代码，以及支持 **SWR** 和 **ReactQuery** 钩子生成
- 基于 **fe-sdk/permissions.json** 可以生成自己想要的模版代码，比如检测登录，导入接口权限等
