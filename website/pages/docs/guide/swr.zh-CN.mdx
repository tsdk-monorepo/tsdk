...
当导出前端模块的时候，通过配置 [.tsdkrc](./.tsdkrc) 的 [dataHookLib](./tsdkrc#datahooklib) 字段为 **SWR**，就可以顺带生成 SWR 钩子。
...
参考代码：https://github.com/tsdk-monorepo/tsdk-starter/blob/main/next-app/app/built-in-swr/page.tsx

<Callout>
  这里路径 `@/lib/user-api-hooks` 是为了和 `@/lib/user-api` 隔离开，避免 Next.js
  服务端组件报错。
</Callout>
