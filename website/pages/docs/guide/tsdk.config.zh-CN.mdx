...
- `dataHookLib` - 可选值有 `SWR` | `ReactQuery` | `VueQuery` | string[]，生成 `SWR` 或者 `React Query` 钩子；get 方法的接口将生成获取钩子，非 get 的将生成更新钩子。默认不生成。依赖版本：swr@^2.3.3；@tanstack/react-query@^5.67.1
...
`dataHookLib` - 可选值有 `SWR` | `ReactQuery` | `VueQuery` | string[]，生成 `SWR`， `React Query` 或者 `Vue Query` 钩子；get 方法的接口将生成获取钩子，非 get 的将生成更新钩子。默认不生成。依赖版本：swr@^2.3.3；@tanstack/react-query@^5.67.1
