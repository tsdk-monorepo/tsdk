...
- Feat: bump xior to latest version and add `isGet` pass to xior instance options.  
...
- Fix: transform paths process error in some case

### 0.0.34 2024/01/2

- Refactor: `nest-webpack.ts` monorepoRoot use `config` from `tsdk.config.js`
...
- Fix: Generate `React Query` hooks error: `Multiple exports of name 'setQueryClient'`
