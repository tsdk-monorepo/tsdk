...
async function reconfigPkg() {
  ...
  if (!scope[0].startsWith("@") || scope.length <= 1) {
    console.log(
      symbols.info,
      `Add scope name in \`.tsdkrc\` \`packageName\` field.`
    );
  }
  ...
  if (scope === defaultPackageScope) {
    console.log(
      symbols.warning,
      `Update package scope name in \`.tsdkrc\` \`packageName\` field.`
    );
  }

  if (
    pkgContent.publishConfig &&
    pkgContent.publishConfig[defaultPackagePrefix]
  ) {
    pkgContent.publishConfig[`${scope}:registry`] =
      pkgContent.publishConfig[defaultPackagePrefix];
    ...
    console.log(
      symbols.warning,
      `\`package.json\` should have \`publishConfig\``
    );
  } else {
    pkgContent.publishConfig[`${scope}:registry`] =
  ...
  await Promise.all(
    files.map(async (file, idx) => {
      ...
      const filePath = path.join(
        ensureDir,
        file.replace(`${config.baseDir}/`, "src/")
      );
...
export async function syncSharedFiles() {
  ...
  const files = await glob(
    config.sharedDirs.map((i) => path.join(i, `**/*.ts`))
  );
  let indexContent = "";
  await Promise.all(
