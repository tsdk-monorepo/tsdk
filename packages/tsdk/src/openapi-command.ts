...
export async function runOpenapiToApiconfCommand() {
  const idx = process.argv.findIndex((i) => i === '--from-openapi' || i === 'from-openapi');
  ...
  if (openapiScript) {
    ...
    logger.warn(symbols.warning, `\`tsdk from-openapi\` currently only support \`build\` command.`);
  }
}

export async function runApiconfToOpenapiCommand() {
  const idx = process.argv.findIndex((i) => i === '--to-openapi' || i === 'to-openapi');
  const restArgv = process.argv.slice(idx + 1);
